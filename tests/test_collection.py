...
class CollectionTest(unittest.TestCase):
    ...
    def test_refresh_catalogue_cli_publishes_both_covered_providers(self) -> None:
        responses = {
            ...
            "/bil/?aston-martin-db9-volante-aut&vid=442795427": (FLEASING_FIXTURES / "aston-martin-db9.html").read_text(encoding="utf-8"),
            "/bil/?bmw-i4&vid=771869804": (FLEASING_FIXTURES / "bmw-i4.html").read_text(encoding="utf-8"),
            "/terminalen": (TERMINALEN_FIXTURES / "catalogue.html").read_text(encoding="utf-8"),
            "/nye-biler/hyundai/hyundai-inster/pris-og-udstyr": (TERMINALEN_FIXTURES / "inster-price-page.html").read_text(encoding="utf-8"),
            "/api/page/url?url=/nye-biler/hyundai/hyundai-inster/pris-og-udstyr&culture=da-DK": (TERMINALEN_FIXTURES / "inster-price-page.json").read_text(encoding="utf-8"),
        }
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(
                [
                    sys.executable, "-m", "car_picker", "refresh-catalogue", "--dataset", str(path),
                    "--fleasing-catalogue-url", f"http://127.0.0.1:{server.server_port}/biler/",
                    "--terminalen-catalogue-url", f"http://127.0.0.1:{server.server_port}/terminalen",
                ],
    ...
    def test_retries_each_provider_at_most_twice_and_publishes_one_complete_generation(self) -> None:
        ...
        def collect_fleasing() -> list[dict[str, str]]:
        ...
        def collect_terminalen() -> list[dict[str, str]]:
        ...
        self.assertEqual([row["name"] for row in dataset["coverage"]["providers"]], ["Fleasing", "Terminalen"])
        self.assertEqual([row["offerIdentity"] for row in dataset["catalogueOffers"]], ["fleasing:one", "terminalen:one"])

    def test_preserves_the_prior_dataset_when_terminalen_fails_after_fleasing_succeeds(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "catalogue-dataset.json"
            ...
            with self.assertRaisesRegex(RuntimeError, "Terminalen collection failed"):
                refresh_catalogue(
                    ...
                    collect_terminalen=lambda: (_ for _ in ()).throw(OSError("Terminalen unavailable")),
                    generated_at=lambda: "2026-07-22T12:00:00Z",
                    sleep=lambda _: None,
                )
            self.assertEqual(path.read_text(encoding="utf-8"), prior)

    def test_preserves_the_prior_dataset_when_validation_rejects_a_partial_candidate(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "catalogue-dataset.json"
            prior = '{"schemaVersion":"catalogue-dataset/v1","marker":"prior"}'
            path.write_text(prior, encoding="utf-8")
...
def fixture_server(responses: dict[str, str]) -> ThreadingHTTPServer:
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:
            ...
            self.send_header("Content-Type", "application/json; charset=utf-8" if self.path.startswith("/api/") else "text/html; charset=utf-8")
            self.end_headers()
