...
class TerminalenAdapterTest(unittest.TestCase):
    def test_collects_each_explicit_private_vat_inclusive_configuration_from_its_same_path_api_response(self) -> None:
        ...
        candidates = TerminalenAdapter(client, retrieved_at="2026-07-22T12:00:00Z").collect(CATALOGUE_URL)
        ...
        self.assertTrue(all(identity.startswith("terminalen:HY_INSTER:private-") for identity in identities))
        self.assertEqual([candidate["admissionStatus"] for candidate in candidates], ["quarantined", "quarantined"])
        ...
        self.assertEqual(candidates[0]["normalEndMechanism"]["value"], "return_to_provider")
        ...
        self.assertTrue(all(event["amountBasis"] == "including_vat" for event in candidates[0]["baseCashFlowStream"]))
        self.assertEqual(candidates[0]["serviceArrangements"]["value"][0]["scope"], "alle fabriksanbefalede services")
        self.assertEqual(
            candidates[0]["exclusions"]["value"],
            [
                {"category": "co2_fee", "treatment": "excluded", "scope": "CO2-afgift"},
                {"category": "insurance", "treatment": "excluded", "scope": "forsikring"},
                {"category": "tyres", "treatment": "excluded", "scope": "ekstra dæk"},
            ],
        )
        self.assertEqual(candidates[0]["sourceMetadata"]["documents"][2]["sourceUrl"], API_URL)

    def test_rejects_an_api_response_for_another_model_path(self) -> None:
        payload = json.loads((FIXTURES / "inster-price-page.json").read_text(encoding="utf-8"))
        ...
        with self.assertRaisesRegex(ValueError, "same model-price path"):
            TerminalenAdapter(client, retrieved_at="2026-07-22T12:00:00Z").collect(CATALOGUE_URL)

    def test_preserves_missing_vat_and_mileage_as_not_stated(self) -> None:
        payload = json.loads((FIXTURES / "inster-price-page.json").read_text(encoding="utf-8"))
        legal = payload["grid"][0]["content"][3]
        legal["text"] = "<p>Priseksemplerne er baseret på 36 mdr. Samlet betaling er inkl. service.</p>"
        client = FixtureHttpClient({
            CATALOGUE_URL: (FIXTURES / "catalogue.html").read_text(encoding="utf-8"),
            PRICE_URL: (FIXTURES / "inster-price-page.html").read_text(encoding="utf-8"),
            API_URL: json.dumps(payload),
        ...
        candidate = TerminalenAdapter(client, retrieved_at="2026-07-22T12:00:00Z").collect(CATALOGUE_URL)[0]

        self.assertEqual(candidate["annualMileageKm"]["state"], "not_stated")
        self.assertTrue(all(event["amountBasis"] == "not_stated" for event in candidate["baseCashFlowStream"]))

    def test_distinguishes_cards_with_the_same_title_using_selected_source_values(self) -> None:
        payload = json.loads((FIXTURES / "inster-price-page.json").read_text(encoding="utf-8"))
        ...
        self.assertEqual(len({candidate["sourceLocalConfigurationKey"] for candidate in candidates}), 2)

    def test_rejects_a_malformed_private_card_instead_of_publishing_a_partial_model(self) -> None:
        payload = json.loads((FIXTURES / "inster-price-page.json").read_text(encoding="utf-8"))
        card = payload["grid"][0]["content"][2]["columns"][1]
        card["text"] = card["text"].replace("Samlet betaling i perioden: 151.395 kr.", "")
        client = FixtureHttpClient({
            CATALOGUE_URL: (FIXTURES / "catalogue.html").read_text(encoding="utf-8"),
            PRICE_URL: (FIXTURES / "inster-price-page.html").read_text(encoding="utf-8"),
            API_URL: json.dumps(payload),
