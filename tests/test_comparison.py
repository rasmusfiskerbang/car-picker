...
class ComparisonCalculationTest(unittest.TestCase):
    def test_calculates_normal_completion_values_with_refunds_without_exposures_in_the_base_stream(
        self,
    ) -> None:
        offer = {
        ...
        self.assertEqual(
            values["upfrontCashRequirement"], {"state": "known", "valueDkk": 16000}
        )
        self.assertEqual(
            values["nominalBaseOutlay"], {"state": "known", "valueDkk": 30000}
        )
        self.assertEqual(
            values["nominalMonthlyEquivalent"], {"state": "known", "valueDkk": 2500}
        )

    def test_keeps_independent_values_when_the_term_blocks_only_the_monthly_equivalent(
        self,
    ) -> None:
        offer = {
        ...
        self.assertEqual(
            values["upfrontCashRequirement"], {"state": "known", "valueDkk": 12000}
        )
        self.assertEqual(
            values["nominalBaseOutlay"], {"state": "known", "valueDkk": 12000}
        )
        self.assertEqual(
    ...
    def test_monthly_equivalent_rounds_the_full_term_division_to_a_whole_dkk(
        self,
    ) -> None:
        offer = {
        ...
        self.assertEqual(
            values["nominalMonthlyEquivalent"], {"state": "known", "valueDkk": 4}
        )
    ...
    def test_monthly_equivalent_names_both_missing_stream_and_term(self) -> None:
        ...
        self.assertEqual(
            ...
            {
                "state": "not_stated",
                "blockingFacts": ["baseCashFlowStream", "termMonths"],
            },
        )

    def test_rejects_non_base_or_out_of_order_events_from_the_base_cash_flow_stream(
        self,
    ) -> None:
        ...
        self.assertEqual(
            excluded_values["nominalBaseOutlay"],
            {"state": "not_stated", "blockingFacts": ["baseCashFlowStream"]},
        )
        self.assertEqual(
            out_of_order_values["upfrontCashRequirement"],
            {"state": "not_stated", "blockingFacts": ["baseCashFlowStream"]},
        )

    def test_keeps_the_known_upfront_requirement_when_a_later_mandatory_payment_is_missing(
        self,
    ) -> None:
        offer = {
            "termMonths": {"state": "known", "value": 12},
