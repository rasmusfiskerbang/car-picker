...
This prototype does not reopen the decisions already made by the rewrite map:
...
- Materialized results are validated against their normalized inputs and never become independently editable authorities.
...
`providers` is the dated copy of Provider Registry records. `offers` contains normalized source facts and validated materialized results. `quarantinedCandidates` contains only identity, provider, canonical source, admission reasons, and the evidence for those reasons. Provider participation and counts are derived from these arrays; designated source scope remains in Provider Adapter code and source audits.

Comparison Availability is expressed directly by each materialized result:

```text
MaterializedResult<T> = Available<T> | Unavailable
```

There is no separate operation-readiness map that could disagree with a result. Each materialized result is either `available` with a value or `unavailable` with reasons. Ordinary facts use the related but distinct union `Known<T> | UnavailableFact`, because fact state describes evidence while result state describes usability.
...
- **Money representation — 2 August 2026:** use ordinary JSON numbers/Python floats for DKK values. Small floating-point rounding errors are acceptable; materialized monetary results are rounded to two decimal places.
- **Result availability — 2 August 2026:** each materialized result carries `state: "available"` and a value or `state: "unavailable"` and explicit reasons. There is no parallel readiness structure and no whole-Offer completeness status.
...
- **No Registration-tax Treatment — 4 August 2026:** remove registration-tax treatment from v1. The product neither calculates nor filters by this contract detail, and advertised payments already enter the Base Cash-flow Stream; retaining it would add an unused comparison fact.
...
- A missing mileage allowance blocks mileage filtering for one Offer without blocking its known upfront cash requirement.
